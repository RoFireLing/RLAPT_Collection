...
//        RLAPT_rlapts rlapt_s = new RLAPT_rlapts();
//        for (int i = 0; i < 10; i++) {
//            rlapt_s.testing("CUBS", 500, i);
//        }
//    }
...
//            } else break;
        ...
        for (int i = 0; i < Constant.repeatNumber; i++) {
            System.out.println("RL-APT4" + objectName + "使用rlapts:" +
                    "执行第" + String.valueOf(i + 1) + "次测试：");

            //初始化RLAPT
            RLAPT_S rlapt_s = new RLAPT_S();

            //初始化测试剖面
            rlapt_s.initializeRLAPT(Constant.getPartitionNumber(objectName));
            ...
            for (int j = 0; j < Constant.number; j++) {
                ...
                if (counter == 1) {
                    ...
                    nextpartitionIndex = rlapt_s.nextPartition4RLAPT(tcnum, partitionIndex, counter);
                } else {
                    partitionIndex = nextpartitionIndex;
                    nextpartitionIndex = nextnextpartitionIndex;
                }
                nextnextpartitionIndex = rlapt_s.nextPartition4RLAPT(tcnum, nextpartitionIndex, counter);
...
//                            }
                ...
                //调整测试剖面
                long startExecuteTestCase = System.nanoTime();
                rlapt_s.adjustRLAPT_S(counter, partitionIndex, nextpartitionIndex, nextnextpartitionIndex, isKilledMutants);
                long endExecuteTestCase = System.nanoTime();
        ...
        //记录均值结果方便查看
        String txtLogName = "RLAPT_S4" + objectName + "#" + tcnum + ".txt";
        ...
        txtLogName = "RLAPT_S4" + objectName + "#" + tcnum + "_Contents.txt";
