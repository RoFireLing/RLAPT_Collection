...
//        RLAPT_rlaptq rlapt_q = new RLAPT_rlaptq();
//        for (int i = 0; i < 10; i++) {
//            rlapt_q.testing("CUBS", 500, i);
//        }
//    }
...
//            } else break;
        ...
        for (int i = 0; i < Constant.repeatNumber; i++) {
            System.out.println("RL-APT4" + objectName + "使用rlaptq:" +
                    "执行第" + String.valueOf(i + 1) + "次测试：");

            //初始化RLAPT
            RLAPT_Q rlapt_q = new RLAPT_Q();

            //初始化测试剖面
            rlapt_q.initializeRLAPT(Constant.getPartitionNumber(objectName));
            ...
            for (int j = 0; j < Constant.number; j++) {
                ...
                nextpartitionIndex = rlapt_q.nextPartition4RLAPT(tcnum, partitionIndex, counter);
...
//                            }
                ...
                //调整测试剖面
                long startExecuteTestCase = System.nanoTime();
                rlapt_q.adjustRLAPT_Q(counter, partitionIndex, nextpartitionIndex, isKilledMutants);
                long endExecuteTestCase = System.nanoTime();
        ...
        //记录均值结果方便查看
        String txtLogName = "RLAPT_Q4" + objectName + "#" + tcnum + ".txt";
        ...
        txtLogName = "RLAPT_Q4" + objectName + "#" + tcnum + "_Contents.txt";
